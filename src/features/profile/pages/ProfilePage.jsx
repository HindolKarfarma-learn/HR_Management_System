  ...
  return (
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[340px_1fr]">
        ...
        <div className="space-y-6">
          <Card><CardHeader title="Personal information" /><CardContent className="grid gap-6 sm:grid-cols-2"><Info icon={Mail} label="Work email" value={profile.email} /><Info icon={Phone} label="Phone" value={profile.phone} /><Info icon={MapPin} label="Address" value={profile.address} /><Info icon={BriefcaseBusiness} label="Employee ID" value={profile.id} /><Info label="Date of birth" value={dayjs(profile.birthDate).format('DD MMMM YYYY')} /><Info label="Gender" value={profile.gender} /></CardContent></Card>
          <div className="grid gap-6 lg:grid-cols-2">
            <Card><CardHeader title="Emergency contact" /><CardContent><p className="font-semibold text-slate-800">{profile.emergencyContact.name}</p><p className="mt-1 text-sm text-slate-500">{profile.emergencyContact.relationship}</p><p className="mt-3 flex items-center gap-2 text-sm text-slate-600"><Phone className="size-4" />{profile.emergencyContact.phone}</p></CardContent></Card>
