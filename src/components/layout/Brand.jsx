...
export function Brand({ compact = false, inverse = false }) {
  return (
    <Link to="/dashboard" className="flex items-center gap-3 rounded-lg">
      ...
      {!compact && (
        <span>
          <span className={`block text-lg font-bold leading-5 ${inverse ? 'text-white' : 'text-slate-900'}`}>PeopleFlow</span>
          <span className={`text-[10px] font-semibold uppercase tracking-[0.18em] ${inverse ? 'text-slate-400' : 'text-slate-400'}`}>HR Management</span>
        </span>
      )}
    </Link>
  );
